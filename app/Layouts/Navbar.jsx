...
const Navbar = () => {
  ...
  const handleLinkClick = () => {
    setTimeout(()=>{setNavActive(false)},500)
  };
    ...
    <header className="w-full h-auto p-2 flex justify-center items-center">
      <div className="md:px-5 lg:px-10 flex justify-between items-center h-auto w-full px-20">
        ...
        <ul className="text-[1.1vw] lg:hidden Font2 w-auto font-light flex justify-center gap-10 items-center">
          <Link onClick={handleLinkClick} className={`${pathname === '/' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/'}>Home</Link>
          <Link onClick={handleLinkClick} className={`${pathname === '/AboutUs' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/AboutUs'}>About Us</Link>
          <Link onClick={handleLinkClick} className={`${pathname === '/Education' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/'}>Education</Link>
          <Link onClick={handleLinkClick} className={`${pathname === '/Price' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/Price'}>Pricing</Link>
          <Link onClick={handleLinkClick} className={`${pathname === '/ContactUs' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/ContactUs'}>Contact Us</Link>
        </ul>
        ...
        <div className={`${navActive ? 'translate-x-[-5%] overflow-hidden sm:translate-x-[-8%]' : 'translate-x-[100%]'} overflow-hidden w-[110vw] ease-linear duration-300 transition-all h-screen bg-[black] top-0 z-20 fixed`}>
          <ul className="text-[20px] h-full text-[#fdc110] flex-col overflow-hidden Font2 w-full font-light flex justify-center gap-10 items-center">
            <Link onClick={handleLinkClick} className={`${pathname === '/' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/'}>Home</Link>
            <Link onClick={handleLinkClick} className={`${pathname === '/AboutUs' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/AboutUs'}>About Us</Link>
            <Link onClick={handleLinkClick} className={`${pathname === '/Education' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/'}>Education</Link>
            <Link onClick={handleLinkClick} className={`${pathname === '/Price' ? 'font-bold scale-125' : 'text-gray-400 scale-100'}`} href={'/Price'}>Pricing</Link>
